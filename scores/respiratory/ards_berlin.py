...
def render():
    ...
    if shared and shared.get('calculator_id') == 'ards_berlin':
        st.info(f"📥 Đã tải kết quả chia sẻ: {shared.get('calculator_name', 'ARDS Berlin Definition')}")
    ...
    **Phân Loại:**
    ...
    if result["all_criteria_met"] and result["severity"]:
        ...
        inputs_dict = {
            "Timing": "Có" if timing else "Không",
            "Chest Imaging": "Có" if chest_imaging else "Không",
            "Origin of Edema": "Có" if origin_of_edema else "Không",
            "Has ABG": "Có" if has_abg else "Không"
        }
        
        if has_abg:
            inputs_dict["PaO2"] = pao2
            inputs_dict["FiO2"] = fio2_input
            inputs_dict["P/F Ratio"] = f"{pf_ratio:.0f}"
        else:
            inputs_dict["Oxygenation Mild"] = "Có" if oxygenation_mild else "Không"
            inputs_dict["Oxygenation Moderate"] = "Có" if oxygenation_moderate else "Không"
            inputs_dict["Oxygenation Severe"] = "Có" if oxygenation_severe else "Không"
        
        results_dict = {
            "Diagnosis": result['diagnosis'],
            "Severity": result.get('severity', 'N/A'),
            "Criteria Met": f"{len(result.get('criteria_met', []))}/4"
        }
        ...
        # History section
        render_history_ui(calculator_id="ards_berlin", show_actions=True)
    ...
    references = get_references("ARDS Berlin Definition")
    if references:
