...
def render():
    ...
    if shared and shared.get('calculator_id') == 'parkland':
        st.info(f"📥 Đã tải kết quả chia sẻ: {shared.get('calculator_name', 'Parkland Formula')}")
    ...
    if st.button("💧 Tính lượng dịch", type="primary", use_container_width=True):
        ...
        **4. Tránh:**
        ...
        results_dict = {
            "Total 24h": f"{result['total_24h']:.0f} ml",
            "First 8h": f"{result['first_8h']:.0f} ml",
            "Next 16h": f"{result['next_16h']:.0f} ml",
            "Rate First 8h": f"{result['rate_first_8h']:.0f} ml/h",
            "Rate Next 16h": f"{result['rate_next_16h']:.0f} ml/h"
        }
        ...
        # History section
        render_history_ui(calculator_id="parkland", show_actions=True)
    ...
    st.markdown("---")
    references = get_references("Parkland Formula")
    if references:
