...
def render():
    ...
    st.markdown(f"<h3 style='text-align: center; color: {COLORS['success']};'>🩹 PASI Score</h3><p style='text-align: center;'><em>Mức độ nặng vẩy nến</em></p>", unsafe_allow_html=True)
    ...
    if st.button("🔬 Tính PASI", type="primary", use_container_width=True):
        ...
        if total < 10: severity = "Nhẹ"; color = COLORS["success"]; icon = "🟢"
        elif total < 20: severity = "Trung bình"; color = COLORS["warning"]; icon = "🟠"
