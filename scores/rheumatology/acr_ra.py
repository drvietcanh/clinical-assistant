...
def render():
    ...
    st.markdown(f"<h3 style='text-align: center; color: {COLORS['success']};'>🦴 ACR/EULAR RA Classification</h3><p style='text-align: center;'><em>Tiêu chuẩn phân loại viêm khớp dạng thấp</em></p>", unsafe_allow_html=True)
    ...
    if st.button("🔬 Đánh giá ACR/EULAR", type="primary", use_container_width=True):
        if total >= 6: 
            ...
            render_score_result(
                title="Kết quả ACR/EULAR",
                score=f"{total}/10",
                interpretation="**Đáp ứng tiêu chuẩn RA**\n\nCó thể chẩn đoán viêm khớp dạng thấp",
