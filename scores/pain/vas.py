...
def render():
    ...
    if shared and shared.get('calculator_id') == 'vas':
        st.info(f"📥 Đã tải kết quả chia sẻ: {shared.get('calculator_name', 'VAS')}")
        if 'shared_inputs' not in st.session_state:
            st.session_state['shared_inputs'] = shared.get('inputs', {})
    
    st.markdown(f"""
    <h2 style='text-align: center; color: {COLORS['success']};'>📏 VAS - Visual Analogue Scale</h2>
    ...
    if st.button("📊 Đánh giá", type="primary", use_container_width=True):
        ...
        render_score_result(
            title="Kết quả VAS",
            score=f"{vas_value} mm",
            interpretation=f"{severity} (Tương đương NRS {nrs_equivalent}/10)",
            color=color,
            icon=icon
        )
