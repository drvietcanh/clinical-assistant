...
def render_hero(
    ...
    """Render hero section with gradient background (supports optional icon)."""
    ...
    if icon:
        ...
        st.markdown(f"""
            <div style="
                ...
                box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
            ">
                <h1 style="margin: 0 0 0.5rem 0; font-size: 2.5rem; font-weight: 700;">{title}</h1>
                <p style="margin: 0 0 0.5rem 0; font-size: 1.2rem; opacity: 0.9;">{subtitle}</p>
                <p style="margin: 0; font-size: 1.1rem; line-height: 1.6; opacity: 0.95;">{description}</p>
            </div>
        """, unsafe_allow_html=True)
